...
Routine to generate a leaflet animation of drifter tracks for the web
@author: Xavier
Minor modifications by JiM in late Aug 2020 on the EMOLT machine
"""
import pandas as pd
...
########
#  HARDCODES
start_yr=20 # as in 2020

#f = urllib.request.urlopen("http://www.nefsc.noaa.gov/drifter/")
#f = urllib.request.urlopen("http://apps-nefsc.fisheries.noaa.gov/drifter/")
f = urllib.urlopen("http://apps-nefsc.fisheries.noaa.gov/drifter/")# JiM reemoved "request" on 9/7/2020
...
for csv in drift_html:
    ...
    try:
        ...
        df=pd.read_csv('https://apps-nefsc.fisheries.noaa.gov/drifter/'+csv+'.csv',index_col=[0])  # read drifter csv file changed by JiM 28 Aug 2020 and added '.csv' 7 Sep 2020
    ...
    for x in range(len(names)): #for each drifter
            ...
            if type(df1.index[0])==numpy.int64:
             if int(str(df1.index[0])[:2])>=start_yr: # this was HARDCODED to >=2016 but we change it on July 20, 2018 to 2017 and on July 29, 2020 to 2020
